...
%%% using decision-tree regression (one tree per output) across multiple
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Channel-wise exclusion for RUL-related evaluation
            % Since RUL is included, remove samples from the same hardware channel
            % as the test sample to avoid leakage across channel-specific histories.
            CountSamChannel = 0;
            ...
            % Training set (after channel filtering)
            In_Train = Temp_F';
            Out_Train = Temp_O';

            % Ensure 2-D shapes are consistent for fitrtree and prediction calls
            ...
            % 2. Train decision-tree regressors (one tree per output dimension)
            trees = cell(6, 1);
            ...
            % Clear per-fold training buffers to limit memory growth
        ...
        % Scatter plots: ground truth vs prediction for the test set only.
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('DT_Result_3_50_Y_Test_%d.mat',CountSV);
