...
%%% voltage setpoints. For RUL-related evaluation, training samples are
%%% restricted to cells from different test channels, and prediction results
%%% are saved for each setpoint.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Construct test sample (single held-out cell)
            Temp_F = Feature_Nor;
            Temp_O = Output;
            In_Test(:,IndexData) = Temp_F(IndexData,:)';
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';

            % Channel-restricted training set (exclude samples from the same channel)
            % This avoids information leakage in RUL-related evaluation.
            CountSamChannel = 0;
...
%         end
    ...
    currentFile = sprintf('PLSR_Result_3_50_Y_Test_%d.mat',CountSV);
