...
%%% for SCU3 Dataset #3, then run repeated leave-one-out BPNN (feedforward NN)
%%% across multiple voltage setpoints and save prediction results.
%%% Note: For Dataset #3, the training set is restricted by channel to avoid
%%% leakage for the RUL-related task (samples from the same channel as the
%%% test sample are excluded from training).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 13:-1:1  
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
           ...
           % Channel-based training restriction (Dataset #3)
           % For RUL-related prediction, exclude samples from the same channel as the test sample
           CountSamChannel = 0;
            ...
            % Ensure 2-D shapes are consistent for newff/train/sim
            ...
            %% BPNN (feedforward neural network) training
            % Network uses tansig hidden layers and purelin output layer, trained with trainlm.
    ...
    currentFile = sprintf('BPNN_Result_3_50_Y_Test_%d.mat',CountSV);
