...
%%% This script: KNN baseline on SCU3 Dataset #3 using ECM feature channels.
%%% It constructs six scaled health indicators (targets), normalizes inputs
%%% and outputs, applies a channel-held-out rule (remove same-channel samples)
%%% for each leave-one-out test sample, repeats training/prediction 100 times,
%%% and saves Y_Test for 13 relaxation-voltage setpoints.
%%% Notes:
...
%%% - For Dataset #3, training excludes samples from the same hardware channel
%%%   as the test sample to reduce leakage in RUL-related evaluation.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% KNN: channel-held-out leave-one-out, repeated 100 times, for 13 setpoints
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Initialize per-sample split containers
            Temp_F = Feature_Nor;
            Temp_O = Output;
            ...
            % Channel-held-out training split (exclude all samples from test channel)
            CountSamChannel = 0;
            ...
            % Predictions on train/test (train outputs cleared later to reduce memory)
            ...
            % Reduce per-iteration memory footprint
            clear Y_Train Out_Train SamChannelIndx
        end
        ...
        % Denormalize test outputs back to physical scales
    ...
    currentFile = sprintf('KNN_Result_3_50_Y_Test_%d.mat', CountSV);
