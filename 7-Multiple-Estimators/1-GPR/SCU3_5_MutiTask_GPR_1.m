...
%%% This script: Train and evaluate Gaussian Process Regression (GPR) models
%%% for SCU3 Dataset #1 using repeated leave-one-out validation across
%%% multiple voltage setpoints, and save prediction results for each setpoint.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% (these are not the final [0,1] normalization used by GPR training).
Capa = Capa/3.5;
Life = Life;
ERate = ERate/89;
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes are consistent for fitrgp and predict
            ...
            % Train one GPR model per task (six tasks)
        ...
        % Result visualization (scatter plots): train and LOOCV test
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('GPR_Result_1_70_Y_Test_%d.mat',CountSV);
