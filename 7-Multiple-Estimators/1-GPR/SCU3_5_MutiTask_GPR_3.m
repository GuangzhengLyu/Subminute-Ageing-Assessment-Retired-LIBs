...
%%% for SCU3 Dataset #3, then run repeated leave-one-out GPR across multiple
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % For RUL-related evaluation, remove samples from the same channel as the test sample
            CountSamChannel = 0;
            ...
            % Keep memory usage stable (as in original script intent)
        ...
        % Result visualization (scatter plots): LOOCV test only
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('GPR_Result_3_50_Y_Test_%d.mat',CountSV);
