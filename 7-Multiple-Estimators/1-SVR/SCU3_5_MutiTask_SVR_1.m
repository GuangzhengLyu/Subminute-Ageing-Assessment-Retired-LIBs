...
%%% This script: Train and evaluate SVR (RBF kernel, libsvm) for SCU3 Dataset #1
...
% (these are not the final [0,1] normalization used by SVR).
Capa = Capa/3.5;
Life = Life;
ERate = ERate/89;
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % For each output dimension, a separate SVR is trained.
            % Note: this reproduces the original structure without altering logic.
            for IndTask = 1:6

                % 1) Grid search for best (C, gamma) using v-fold CV
                ...
                % 2) Train SVR model using best parameters
                cmd = [' -t 2', ...
                       ' -c ', num2str(bestc), ...
                       ' -g ', num2str(bestg), ...
                       ' -s 3 -p 0.01'];
        ...
        % Optional visualization (kept enabled as in original script)
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('SVR_Result_1_70_Y_Test_%d.mat', CountSV);
