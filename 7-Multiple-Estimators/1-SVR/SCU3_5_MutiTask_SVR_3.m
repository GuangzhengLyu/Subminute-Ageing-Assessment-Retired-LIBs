...
%%% This script: Train and evaluate SVR (RBF kernel, libsvm) for SCU3 Dataset #3
%%% using repeated leave-one-out across 13 voltage setpoints. For each setpoint,
%%% the script performs setpoint-dependent min-max normalization of relaxation
%%% features, min-max normalization of multi-task outputs, then runs 100 repeated
%%% LOOCV trials and saves Y_Test for each setpoint.
%%%
%%% Note: For Dataset #3, training samples are restricted to cells from
%%% different test channels than the test cell (channel-wise exclusion),
%%% to reduce information leakage when predicting life/RUL-related targets.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% SVR (libsvm) with repeated leave-one-out across setpoints
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Construct test sample (leave-one-out)
            Temp_F = Feature_Nor;
            Temp_O = Output;
            In_Test(:,IndexData) = Temp_F(IndexData,:)';
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';

            % Channel-wise exclusion for Dataset #3:
            % remove all samples from the same channel as the test sample
            CountSamChannel = 0;
            ...
            % Ensure 2-D shapes are consistent for libsvm calls
            ...
            %% SVR model training (RBF kernel) with grid search on (C, gamma)
            for IndTask = 1:6

                % Grid search for best (C, gamma) using v-fold CV
                ...
                for ii = 1:m
                    for jj = 1:n
                        cmd = ['-v ',num2str(v),' -t 2', ...
                               ...
                               ' -g ',num2str(2^g(ii,jj)), ...
                               ' -s 3 -p 0.1'];
                        cg(ii,jj) = svmtrain(Out_Train(IndTask,:)', In_Train', cmd);
                        ...
                        % Tie-break: prefer smaller C when CV errors are nearly equal
                        if abs(cg(ii,jj) - error) <= eps && bestc > 2^c(ii,jj)
                            error = cg(ii,jj);
                            bestc = 2^c(ii,jj);
                            bestg = 2^g(ii,jj);
                        end
                ...
                % Train SVR model using best parameters
                cmd = [' -t 2', ...
                       ' -c ',num2str(bestc), ...
                       ' -g ',num2str(bestg), ...
                       ' -s 3 -p 0.01'];
                model{IndTask} = svmtrain(Out_Train(IndTask,:)', In_Train', cmd);

            end

            %% SVR prediction
            for IndTask = 1:6
                Y_Train(IndTask,:) = svmpredict(Out_Train(IndTask,:)', In_Train', model{IndTask});
                Y_Test(CountRP,IndTask,IndexData) = svmpredict(Out_Test(CountRP,IndTask,IndexData)', In_Test(:,IndexData)', model{IndTask});
            end

            toc
            clear Y_Train Out_Train model SamChannelIndx
        end
        ...
        % De-normalize predictions and labels back to original output scales
        ...
        % Optional visualization (enabled as in original script)
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('SVR_Result_3_50_Y_Test_%d.mat', CountSV);
