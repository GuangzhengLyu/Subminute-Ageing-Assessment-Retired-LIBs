...
%%% This script: Post-process XGBoost results for SCU3 Dataset #1.
%%% It reconstructs normalized ground-truth outputs, loads saved XGBoost
%%% predictions across voltage setpoints, computes per-task RMSE over
%%% repetitions, and visualizes the mean RMSE versus terminal-voltage setpoint.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% and expanded health indicators (all in the same normalized scales used by XGBoost).
CountData = 0;
...
% Load XGBoost prediction files for each setpoint (i = 1..13) and compute RMSE
% for each output dimension over repeated runs (j = 1..100).
for i = 1:13
    currentFile = sprintf('XGBoost_Result_1_70_Y_Test_%d.mat',i);
    load(currentFile)
    for j = 1:100
...
save M_RMSE_XGBoost_1.mat M_RMSE