...
%%% for SCU3 Dataset #3, then run repeated leave-one-out Random Forest (RF)
%%% regression across multiple voltage setpoints and save prediction results.
%%% Note: For RUL-related evaluation, training samples from the same channel
%%% as the test sample are excluded to avoid channel leakage.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 13:-1:1  
    ...
    for CountRP = 1:30
        for IndexData = 1 :length(Feature_Nor)
            ...
            % Construct initial split: one sample for test, remaining for candidate training pool
            Temp_F = Feature_Nor;
            Temp_O = Output;
            In_Test(:,IndexData) = Temp_F(IndexData,:)';
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';
            
           % For RUL-related evaluation, exclude samples from the same channel as the test sample
           % to avoid leakage due to shared operating histories within one channel.
           CountSamChannel = 0;
            ...
            % Ensure 2-D shapes are consistent for TreeBagger input/output
            ...
            % Random Forest regression is trained independently for each output dimension.
            trees = 100;                                      % number of decision trees
            leaf  = 5;                                        % minimum leaf size
            OOBPrediction = 'on';                             % enable out-of-bag predictions
            ...
            % Prediction (held-out test sample) in normalized space
        ...
        % Scatter plots of prediction vs. label for the test set
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('RF_Result_3_50_Y_Test_%d.mat',CountSV);
