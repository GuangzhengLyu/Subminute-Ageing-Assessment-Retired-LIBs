...
%%% outputs for SCU3 Dataset #3, then run repeated leave-one-out evaluation
%%% using a Bayesian-regularized neural network (BNN, trainbr) across multiple
%%% voltage setpoints and save prediction results for each setpoint.
%%% Note: For Dataset #3, training samples are additionally filtered by
%%% channel to avoid using same-channel data when the task involves RUL.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% BNN (Bayesian-Regularized Neural Network)
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
           ...
           % RUL-related constraint:
           % Only samples from different channels are used as training samples
           CountSamChannel = 0;
            ...
            % Ensure 2-D shapes are consistent for Neural Network Toolbox
            ...
            % 2. Create Bayesian-regularized neural network (trainbr)
            % Architecture: two hidden layers with 10 and 5 neurons.
            net = fitnet([10, 5], 'trainbr');
            net.trainParam.showWindow = false;
            net.trainParam.epochs = 20;

            % 3. Train on the current training split
    ...
    currentFile = sprintf('BNN_Result_3_50_Y_Test_%d.mat',CountSV);
