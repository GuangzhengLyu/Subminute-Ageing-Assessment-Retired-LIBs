...
%%% SCU3 Dataset #3, then run repeated leave-one-out PLSR (max 4 components)
...
% setpoint-specific bounds, then run repeated evaluation.
for CountSV = 13:-1:1  
    ...
    % For RUL-related prediction, only samples from different channels are used
    % as training samples relative to the current test sample.
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
            ...
            % Initialize LOOCV split (test sample is fixed by IndexData)
            Temp_F = Feature_Nor;
            Temp_O = Output;
           ...
           CountSamChannel = 0;
            ...
            % 2. Train PLSR (component number capped at 4)
            ncomp = min(4, size(In_Train,1));
...
%         end
    ...
    currentFile = sprintf('PLSR4_Result_3_50_Y_Test_%d.mat',CountSV);
