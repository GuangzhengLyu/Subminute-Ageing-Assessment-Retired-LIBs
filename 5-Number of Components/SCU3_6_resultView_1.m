...
for InDexLV = 1:6
    for i = 1:13
        currentFile = sprintf('./%d/PLSR%d_Result_1_70_Y_Test_%d.mat',InDexLV,InDexLV,i);
        load(currentFile)
        for j = 1:100
...
figure,plot(Error(1,:),'o-'),axis([0,7,0.0285,0.034])
figure,plot(Error(2,:),'s-'),axis([0,7,52,56.5])
figure,plot(Error(3,:),'d-'),axis([0,7,0.0107,0.0116])
figure,plot(Error(4,:),'>-'),axis([0,7,0.0178,0.0193])
figure,plot(Error(5,:),'^-'),axis([0,7,0.0272,0.029])
figure,plot(Error(6,:),'<-'),axis([0,7,0.064,0.072])
