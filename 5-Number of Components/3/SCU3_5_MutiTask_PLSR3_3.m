...
%%% voltage setpoint using a fixed latent component number (ncomp = 3).
%%% For RUL-related evaluation, training samples are restricted to different
%%% channels than the held-out test sample. Test predictions are saved per
%%% setpoint for downstream sensitivity analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 13:-1:1
    ...
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
            ...
            % Construct split: one sample for test, remaining for training
            Temp_F = Feature_Nor;
            Temp_O = Output;
            In_Test(:,IndexData) = Temp_F(IndexData,:)';
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';

           % RUL-related setup: remove samples from the same channel as the test sample
           CountSamChannel = 0;
...
%         end
    ...
    currentFile = sprintf('PLSR3_Result_3_50_Y_Test_%d.mat',CountSV);
