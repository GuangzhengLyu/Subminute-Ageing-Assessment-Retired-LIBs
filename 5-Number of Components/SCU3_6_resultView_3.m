...
for InDexLV = 1:6
    for i = 1:13
        currentFile = sprintf('./%d/PLSR%d_Result_3_50_Y_Test_%d.mat',InDexLV,InDexLV,i);
        load(currentFile)
        for j = 1:100
...
figure,plot(Error(1,:),'o-'),axis([0,7,0.0145,0.0195])
figure,plot(Error(2,:),'s-'),axis([0,7,182,185])
figure,plot(Error(3,:),'d-'),axis([0,7,0.01,0.0135])
figure,plot(Error(4,:),'>-'),axis([0,7,0.045,0.066])
figure,plot(Error(5,:),'^-'),axis([0,7,0.087,0.106])
figure,plot(Error(6,:),'<-'),axis([0,7,0.023,0.036])
