...
%%% This script: Assemble SCU3 Dataset #3 samples, scale/normalize multi-
%%% dimensional health outputs, then run repeated leave-one-out PLSR for each
%%% voltage setpoint using a fixed latent component number (ncomp = 2).
%%% For channel-aware evaluation, samples from the same channel as the test
%%% sample are excluded from the training set, and test-set predictions are
%%% saved for downstream sensitivity analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% (these are not the final [0,1] min-max normalization used by PLSR).
Capa = Capa/3.5;
Life = Life;
ERate = ERate/89;
...
%% PLSR2
% For each voltage setpoint (CountSV), normalize input features using
...
for CountSV = 13:-1:1
    ...
    %% Repeated leave-one-out evaluation (channel-aware training exclusion)
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
           ...
           % For RUL-related evaluation, exclude samples from the same channel
           % as the test sample to avoid within-channel leakage.
           CountSamChannel = 0;
            ...
            % Construct training matrices (predictors and multi-output responses)
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            % 2. Train PLSR (latent component number fixed here for sensitivity study)
            ncomp = min(2, size(In_Train,1));
            [~, ~, ~, ~, beta] = plsregress(In_Train', Out_Train', ncomp);

            % 3. Predict on training and held-out test sample
...
%         % Result visualization (kept commented as in original script)
%         for IndOTrain = 1:6
...
%         end
    ...
    currentFile = sprintf('PLSR2_Result_3_50_Y_Test_%d.mat',CountSV);
