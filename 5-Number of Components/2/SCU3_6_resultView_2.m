...
%%% This script: Post-process saved SCU3 Dataset #2 PLSR2 test predictions,
%%% compute RMSE (per task, per voltage setpoint, per repetition), normalize
%%% RMSE by task-specific reference scales, and visualize aggregated trends
%%% across voltage setpoints for latent-component sensitivity analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Filter samples by the ending step flag and compute normalized targets that
% match the output scaling used during model training and saving.
CountData = 0;
for IndexData = 1:length(OneCycle)
    if OneCycle(IndexData).Steps(end) == 30
        ...
        % Expanded health indicators (normalized to unified health-indicator scales)
...
%% Result aggregation and error metrics
% For each voltage setpoint file (i = 1..13), load saved repeated-test
% predictions and compute RMSE for each task across all samples.
for i = 1:13
    currentFile = sprintf('./PLSR2_Result_2_60_Y_Test_%d.mat',i);
    load(currentFile)
    for j = 1:100
...
%% Example scatter plots and error vectors
% Note: j uses the last loop value (j = 100) as in the original script.
...
%% Voltage setpoints and normalized RMSE aggregation
% Uter: terminal-voltage grid for setpoints (kept for reference)
Uter = 3.0:0.1:4.2;

% Normalize mean RMSE across repetitions by task-specific reference scales
...
% Aggregate normalized RMSE across tasks to obtain one curve per setpoint
MyTemp = mean(Temp,1);

%% Visualization of aggregated normalized RMSE across setpoints
