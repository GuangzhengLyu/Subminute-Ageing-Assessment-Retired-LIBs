clear
clc
close all

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Project: PLSR Latent Component Sensitivity Study
%%% This script: Load SCU3 Dataset #2 ground-truth health indicators, read
%%% saved PLSR6 predictions (per voltage setpoint), compute RMSE matrices
%%% across 100 repetitions, and plot aggregated normalized error trends.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% SCU3 Dataset #2
% Load structured single-cycle dataset
load('../../OneCycle_2.mat')

%% Sample construction (ground-truth indicators)
% Filter samples by the ending step flag and construct normalized indicators:
% capacity, life, and expanded health indicators for performance assessment.
CountData = 0;
for IndexData = 1:length(OneCycle)
    if OneCycle(IndexData).Steps(end) == 30
        CountData = CountData+1;
        
        % Define life as the first cycle where discharge capacity drops below 2.1 Ah
        % If the threshold is not reached, use the full trajectory length
        if (min(OneCycle(IndexData).Cycle.DiscCapaAh) <= 2.1)
            Life(CountData,1) = min(find(OneCycle(IndexData).Cycle.DiscCapaAh < 2.1));
        else
            Life(CountData,1) = length(OneCycle(IndexData).Cycle.DiscCapaAh);
        end
        
        % Unified health-indicator scaling (consistent with the training scripts)
        Capa(CountData,1) = OneCycle(IndexData).OrigCapaAh./3.5;
        
        % Expanded health indicators (scaled to comparable forms)
        ERate(CountData,1) = OneCycle(IndexData).Cycle.EnergyRate(2)/89;
        CoChRate(CountData,1) = OneCycle(IndexData).Cycle.ConstCharRate(2)/83;
        MindVolt(CountData,1) = (OneCycle(IndexData).Cycle.MindVoltV(1)-2.65)/(3.47-2.65);
        PlatfCapa(CountData,1) = OneCycle(IndexData).Cycle.PlatfCapaAh(1)/1.3;
    end
end

%% Result evaluation
% For each voltage setpoint (i = 1..13), load saved repeated-test predictions
% and compute RMSE for each output dimension across 100 repetitions (j = 1..100).
for i = 1:13
    currentFile = sprintf('./PLSR6_Result_2_60_Y_Test_%d.mat',i);
    load(currentFile)
    for j = 1:100
        RMSE_Capa(i,j) = sqrt(mean((Capa - squeeze(Y_Test(j,1,:))).^2));
        RMSE_Life(i,j) = sqrt(mean((Life - squeeze(Y_Test(j,2,:))).^2));
        RMSE_ERate(i,j) = sqrt(mean((ERate - squeeze(Y_Test(j,3,:))).^2));
        RMSE_CoChR(i,j) = sqrt(mean((CoChRate - squeeze(Y_Test(j,4,:))).^2));
        RMSE_MipV(i,j) = sqrt(mean((MindVolt - squeeze(Y_Test(j,5,:))).^2));
        RMSE_PlatfCapa(i,j) = sqrt(mean((PlatfCapa - squeeze(Y_Test(j,6,:))).^2));
    end
end

%% Scatter plots and error vectors (single repetition index j)
% Note: j here is the loop variable from the previous section; after the loop,
% it remains at its final value (100). The code below reproduces the original behavior.
figure(7),hold on,plot(Capa,squeeze(Y_Test(j,1,:)),'o')
ErCA_1 = Capa-squeeze(Y_Test(j,1,:));
figure(8),hold on,plot(Life,squeeze(Y_Test(j,2,:)),'s')
ErLI_1 = Life-squeeze(Y_Test(j,2,:));
figure(9),hold on,plot(ERate,squeeze(Y_Test(j,3,:)),'>')
ErER_1 = ERate-squeeze(Y_Test(j,3,:));
figure(10),hold on,plot(CoChRate,squeeze(Y_Test(j,4,:)),'d')
ErCR_1 = CoChRate-squeeze(Y_Test(j,4,:));
figure(11),hold on,plot(MindVolt,squeeze(Y_Test(j,5,:)),'^')
ErMV_1 = MindVolt-squeeze(Y_Test(j,5,:));
figure(12),hold on,plot(PlatfCapa,squeeze(Y_Test(j,6,:)),'>')
ErPC_1 = PlatfCapa-squeeze(Y_Test(j,6,:)); 
  
%% Voltage setpoint axis (kept as in original script)
Uter = 3.0:0.1:4.2;

%% Aggregated normalized RMSE trend
% Compute mean RMSE across repetitions for each setpoint, then normalize each
% indicator by a fixed scale to make them comparable before averaging.
Temp(1,:) = mean(RMSE_Capa,2)/0.0017;
Temp(2,:) = mean(RMSE_Life,2)/167.3298;
Temp(3,:) = mean(RMSE_ERate,2)/0.0141;
Temp(4,:) = mean(RMSE_CoChR,2)/0.0553;
Temp(5,:) = mean(RMSE_MipV,2)/0.0713;
Temp(6,:) = mean(RMSE_PlatfCapa,2)/0.0681;

% Average normalized RMSE across all output dimensions
MyTemp = mean(Temp,1);

%% Plot summary curve across setpoints
figure(1),hold on,plot(MyTemp,'-o'),box on,grid on