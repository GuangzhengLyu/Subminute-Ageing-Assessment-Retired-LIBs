...
%%% SCU3 Dataset #3, then run repeated leave-one-out PLSR (max 6 components)
%%% across multiple voltage setpoints and save prediction results per setpoint.
%%% Note: For RUL-related training, samples from the same test channel are
%%% excluded from the training set to avoid within-channel leakage.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 13:-1:1  
    ...
    %% Repeated leave-one-out evaluation with channel-based exclusion
    % For each test sample (IndexData), remove all samples from the same
    % channel from the training set before fitting PLSR.
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
            ...
            % Prepare full copies for splitting
            Temp_F = Feature_Nor;
            Temp_O = Output;
            ...
            % as the current test sample when constructing the training set.
            CountSamChannel = 0;
            ...
            % 2. Train PLSR (component number capped at 6)
            ncomp = min(6, size(In_Train,1));
...
%         end
    ...
    currentFile = sprintf('PLSR6_Result_3_50_Y_Test_%d.mat',CountSV);
