...
%%% This script: Build relaxation-feature tensors and normalized outputs for
%%% SCU3 Dataset #3, then run repeated leave-one-out PLSR (max 5 components)
%%% across multiple voltage setpoints. For RUL-related prediction, training
%%% samples are restricted to those from different channels than the test
%%% sample. Results are saved per setpoint.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% setpoint-specific bounds, then run repeated evaluation. Training samples
% are restricted by channel to reduce leakage for RUL-related prediction.
for CountSV = 13:-1:1  
    ...
    %% Repeated evaluation with channel-aware training selection
    % For RUL prediction, only samples from different channels are used as
    % training samples relative to the current test sample.
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
            ...
            % Initialize split (test sample is fixed by IndexData)
            Temp_F = Feature_Nor;
            Temp_O = Output;
            In_Test(:,IndexData) = Temp_F(IndexData,:)';
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';
            
           % For RUL prediction, exclude samples from the same channel as the test sample
           CountSamChannel = 0;
            ...
            % 2. Train PLSR (component number capped at 5)
...
%         end
    ...
    currentFile = sprintf('PLSR5_Result_3_50_Y_Test_%d.mat',CountSV);
