...
%%% This script: Load SCU3 Dataset #2, build normalized ground-truth outputs,
...
for i = 1:13
    currentFile = sprintf('./Result/PLSR_Result_2_60_Y_Test_%d.mat',i);
    load(currentFile)
...
figure(7),hold on,plot(Capa,squeeze(Y_Test(j,1,:)),'o'),ErCA_2 = Capa-squeeze(Y_Test(j,1,:));
figure(8),hold on,plot(Life,squeeze(Y_Test(j,2,:)),'s'),ErLI_2 = Life-squeeze(Y_Test(j,2,:));
figure(9),hold on,plot(ERate,squeeze(Y_Test(j,3,:)),'>'),ErER_2 = ERate-squeeze(Y_Test(j,3,:));
figure(10),hold on,plot(CoChRate,squeeze(Y_Test(j,4,:)),'d'),ErCR_2 = CoChRate-squeeze(Y_Test(j,4,:));
figure(11),hold on,plot(MindVolt,squeeze(Y_Test(j,5,:)),'^'),ErMV_2 = MindVolt-squeeze(Y_Test(j,5,:));
figure(12),hold on,plot(PlatfCapa,squeeze(Y_Test(j,6,:)),'>'),ErPC_2 = PlatfCapa-squeeze(Y_Test(j,6,:)); 

% save Error_2.mat ErCA_2 ErLI_2 ErER_2 ErCR_2 ErMV_2 ErPC_2
