...
%%% for SCU3 Dataset #3, then run repeated leave-one-out PLSR across multiple
%%% voltage setpoints with channel-wise exclusion for training, and save
%%% prediction results for each setpoint.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% For Dataset #3, training samples are restricted to different channels than
% the test sample (channel-wise exclusion) to avoid leakage for RUL prediction.
for CountSV = 13:-1:1  
    ...
    for CountRP = 1:100
        for IndexData = 1 :length(Feature_Nor)
           ...
           % Channel-wise exclusion: use only samples from different channels
           % as training data to reduce leakage for RUL-related targets.
           CountSamChannel = 0;
...
%         end
    ...
    currentFile = sprintf('./Result/PLSR_Result_3_50_Y_Test_%d.mat',CountSV);
