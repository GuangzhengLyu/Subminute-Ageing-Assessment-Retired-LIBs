...
Each `SCU_SHAP_k.m` script loads the processed dataset and feature file from the parent directory:
...
- RUL proxy:
  ```
  cycle count to a capacity threshold (dataset-dependent)
  ```
...
Each target is then linearly scaled to `[0,1]` using dataset-specific `Min_Out` and `Max_Out`, and clipped into `[0,1]`.
...
- `SCU_SHAP_1.m` (Dataset #1):
  ...
  y = Output(:,6)
  ```
  → platform-discharge-capacity-based SOH (normalized)
...
- `SCU_SHAP_3.m` (Dataset #3):
  ...
  y = Output(:,5)
  ```
  → mid-point-voltage-based SOH (normalized)

This design focuses the feature-attribution analysis on different service-relevant indicators under different retirement scenarios.
...
- Background set:
  ```
  up to 200 random samples from training data
  ```
- Local explanation:
  ```
  one selected test sample
  ```
- Global importance:
  ```
  mean absolute SHAP over up to 100 random test samples
  ```
...
3. Global SHAP importance  
   ```
   MeanAbsoluteShapley = mean(abs(SHAP),1)
   ```
...
5. Dependence plot  
   ```
   plotDependence(explainerAll, featureName)
   ```

No files are saved by default unless explicit `save` or `exportgraphics` commands are added.
