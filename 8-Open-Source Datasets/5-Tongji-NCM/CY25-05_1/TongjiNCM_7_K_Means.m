...
%%% This script: Perform grouped K-means clustering using estimated health
%%% indicators and relaxation features (with group-wise z-score normalization
%%% and grouped weighted Euclidean distance). Then evaluate within-cluster
%%% trajectory dispersion (STD over cycle index) for multiple engineering
%%% trajectories, summarize relative dispersion ratios, visualize results,
%%% and save the aggregated metrics to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Add custom grouped K-means implementation to path
addpath('./G-K-means')

%% ========== 1. Load data (grouped inputs and model estimation) ==========
load('./OneCycle_TongjiNCM25.mat')  
load('./Feature_ALL_TongjiNCM25.mat')
load('./PLSR_Result_1_70_Y_Test_14_TongjiNCM25.mat')

% Use the mean prediction over repeated runs as the final estimation
Estimation = squeeze(mean(Y_Test,1))';

% Assemble relaxation-feature tensor: (sample, segment, feature-dimension)
Feature(:,:,1) = Uoc;
Feature(:,:,2) = R0;
Feature(:,:,3) = R1;
Feature(:,:,4) = C1;
...
%% Convert to health-indicator groups (for grouped K-means)
Xg = cell(4,1);

for Kind = 1:4
    rng(42); % Fix random seed for reproducibility

    % Group 1: capacity-related estimation (always enabled)
    Xg{1} = Estimation(:,1);   % N×1
    
    % Group 2: life-related estimation (enabled for Kind = 2/3/4)
    ...
    % Group 4: expanded health indicators (enabled for Kind = 4)
    ...
    % Concatenate into a single feature matrix for K-means
    % Manual concatenation is kept to avoid cell2mat dimension mismatch errors
    Xn = [];
    for g = 1:numel(Xg_norm)
        A = Xg_norm{g};
    
        % Force to a 2D matrix
        if isvector(A)
    ...
    % You can tune each group weight to reflect "group importance"
    w = [1, 1, 1, 1];
    ...
    % Custom K-means (k-means++ init + Lloyd updates)
    ...
    title('K-means clustering (performed in full space; PCA only for plotting)');
    
    %% ========== 5. Visualization 2: cluster-center profiles by group ==========
    % Split C back into each group and plot the normalized center profiles
    figure('Name','Cluster-center profiles for each group');
    ...
    % Intra-cluster average distance (group-weighted)
    ...
    %% ========== 7. Trajectory-dispersion evaluation within clusters ==========
    for i = 1:K
    ...
    % Clear intermediate variables for the current Kind loop
...
% Relative dispersion ratios (Kind = 4 vs Kind = 1)
...
% Normalize each metric by the Kind = 1 baseline and assemble a result matrix
...
% Bar plots: normalized dispersion metrics across Kind settings
