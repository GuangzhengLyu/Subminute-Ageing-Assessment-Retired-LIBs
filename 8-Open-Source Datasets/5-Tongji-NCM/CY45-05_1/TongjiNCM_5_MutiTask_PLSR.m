...
%%% leave-one-out PLSR to predict multi-task health indicators and save results.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Sample construction
% Assemble target outputs and engineering-oriented expanded indicators
CountData = 0;
...
for CountSV = 14:-1:14 
    ...
    % For each repetition, leave each sample out once, train on the rest,
    % and predict the held-out sample across all output targets
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % 2. Train PLSR (auto-select the number of components)
    ...
    currentFile = sprintf('PLSR_Result_1_70_Y_Test_%d_TongjiNCM.mat',CountSV);
