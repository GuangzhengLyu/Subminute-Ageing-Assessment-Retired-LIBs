...
%%% This script: Perform grouped, weighted K-means clustering on health
%%% estimation outputs and relaxation features, then evaluate within-cluster
%%% trajectory dispersion (STD over cycles) for multiple engineering signals.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Add grouped K-means utilities (k-means++ + Lloyd with grouped distance)
addpath('./G-K-means')

%% ========== 1. Load grouped data ==========
% Load single-cycle dataset, extracted relaxation features, and PLSR outputs
load('./OneCycle_TongjiNCM.mat')  
load('./Feature_ALL_TongjiNCM.mat')
load('./PLSR_Result_1_70_Y_Test_14_TongjiNCM.mat')

% Use the mean prediction across repetitions as the final estimation (N x 6)
Estimation = squeeze(mean(Y_Test,1))';

% Pack relaxation features into a 3D array for consistent handling
Feature(:,:,1) = Uoc;
Feature(:,:,2) = R0;
Feature(:,:,3) = R1;
Feature(:,:,4) = C1;
...
% Xg is a cell array where each cell is one feature group (N x d_g)
Xg = cell(4,1);

for Kind = 1:4
    % Fix random seed for reproducibility (used by k-means initialization)
    rng(42);

    % Group 1: capacity-based health indicator (N x 1)
    Xg{1} = Estimation(:,1);   % N×1
    
    % Group 2: life-related indicator (enabled for Kind = 2,3,4)
    ...
    % Group 4: expanded performance indicators (enabled for Kind = 4 only)
    ...
    % Standardize each group independently (z-score) to avoid scale dominance
    Xg_norm = cell(G,1);
    for g = 1:G
        [Xg_norm{g}, ~] = zscore_norm(Xg{g});
    end
    
    % Concatenate all normalized groups into a single feature matrix for K-means
    % Manual concatenation is used to avoid cell2mat failures from shape issues
    Xn = [];
    for g = 1:numel(Xg_norm)
        ...
        % If A is accidentally d×N, transpose to N×d
        ...
        if size(A,1) ~= N
            error('Group %d has invalid shape: expected %d×%d, got %d×%d', ...
    ...
    % Group weights: adjust importance of each group in the distance metric
    % Example: emphasize group 3 via w = [1, 1, 2, 0.8]
    w = [1, 1, 1, 1];
    
    opts.maxIter = 200;
    opts.tol = 1e-6;
    opts.nInit = 8;         % multiple initializations, keep the best solution
    opts.verbose = true;
    ...
    %% ========== 4. Visualization 1: PCA projection (2D scatter) ==========
    ...
    title('K-means clustering (clustering in full space; PCA for visualization only)');
    
    %% ========== 5. Visualization 2: cluster-center profiles per group ==========
    % Split the concatenated centers back into groups and plot center profiles
    ...
    % Simple evaluation: mean weighted intra-cluster distance
    ...
    % Compute per-cycle STD within each cluster for multiple engineering signals
    for i = 1:K
    ...
    % Aggregate dispersion metrics across clusters (mean of cluster means)
    ...
    % Clear per-iteration temporary variables (j-index-related variables)
...
% Report relative dispersion reduction from Kind=1 to Kind=4 (selected metrics)
...
%% Normalize results relative to Kind=1 baseline
...
%% Visualization: bar charts for normalized dispersion metrics
...
% Save K-means evaluation result
