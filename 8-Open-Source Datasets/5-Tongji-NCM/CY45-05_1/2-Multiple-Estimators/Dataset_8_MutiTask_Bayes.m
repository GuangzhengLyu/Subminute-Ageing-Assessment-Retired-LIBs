...
%%% leave-one-out Bayesian linear regression (closed-form) to predict all
%%% health indicators and save the test predictions to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out Bayesian linear regression (closed-form)
    for CountRP = 1:100
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    % Get number of features (M) and number of samples (N)
    M = size(p_train, 1);
    N = size(p_train, 2);
    ...
    % Compute posterior mean of coefficients (closed-form)
