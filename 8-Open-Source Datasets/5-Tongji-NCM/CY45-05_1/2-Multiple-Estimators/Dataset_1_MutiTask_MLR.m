...
%%% leave-one-out multi-task MLR (six independent linear models) to predict
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out multi-task MLR
    % Six independent linear models are trained (one per target) for each split
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Train six independent MLR models (one per output)
            ...
            % Predict on held-out test sample (stored for later evaluation)
