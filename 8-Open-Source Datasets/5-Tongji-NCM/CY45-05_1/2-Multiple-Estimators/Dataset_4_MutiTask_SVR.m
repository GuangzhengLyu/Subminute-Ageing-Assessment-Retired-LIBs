...
%%% leave-one-out Support Vector Regression (SVR, RBF kernel). For each task,
%%% SVR hyperparameters (C, gamma) are selected by grid search with CV.
%%% Predictions are inverse-normalized and saved to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% SVR library functions (e.g., LIBSVM)
addpath('./4-SVR');
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out SVR (RBF kernel)
    % For each split, perform grid search (C, gamma) with CV, then train SVR
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Model 1
            % 1. Search best C / gamma by grid + cross-validation
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1'];%2 Gaussian kernel; -v returns CV value; -s 3 SVR
            ...
            % 2. Train SVR with the selected C and gamma
            cmd = [' -t 2',' -c ',num2str(bestc),' -g ',num2str(bestg),' -s 3 -p 0.01'];% use best C/gamma
