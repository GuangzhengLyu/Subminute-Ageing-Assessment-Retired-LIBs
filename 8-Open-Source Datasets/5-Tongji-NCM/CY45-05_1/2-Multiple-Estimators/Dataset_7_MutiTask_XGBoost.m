...
%%% leave-one-out XGBoost regression (six independent models) to predict all
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out XGBoost regression
    % Six independent XGBoost models are trained (one per target) for each split
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            %% XGBoost
            % Parameter set is preserved as provided in the original script
            num_trees = 100;
            params.eta = 0.1;
            ...
            % Train one model per output (multi-task via independent regressors)
