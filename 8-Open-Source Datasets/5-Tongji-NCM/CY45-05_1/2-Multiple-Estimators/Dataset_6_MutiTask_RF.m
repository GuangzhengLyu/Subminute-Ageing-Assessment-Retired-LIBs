...
%%% leave-one-out Random Forest Regression (TreeBagger; six independent models)
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out Random Forest Regression (TreeBagger)
    % Six independent forests are trained (one per target) for each split
    for CountRP = 1:100
