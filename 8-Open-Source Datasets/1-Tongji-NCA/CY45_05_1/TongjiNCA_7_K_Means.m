...
%%% This script: Evaluate sorting/reconfiguration consistency using a
%%% grouped, weighted K-means clustering pipeline. The clustering is built
%%% on (i) estimated capacity/life outputs, (ii) fitted relaxation features,
%%% and (iii) expanded performance indicators. We quantify within-cluster
%%% trajectory dispersion using per-cycle standard deviation statistics of
%%% key trajectories (capacity/energy/charge/time/platform capacity) and
%%% summarize relative dispersion reductions across feature-combination
%%% settings (Kind = 1..4).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Add grouped K-means toolbox/functions
addpath('./G-K-means')
...
% Mean estimation over repeated runs (Y_Test: [runs x outputs x samples])
...
%% Convert to grouped "health indicators" (feature groups)
% Xg is a cell array of groups; each group is an N×d_g matrix
Xg = cell(4,1);

for Kind = 4:-1:1
    rng(42); % Fixed random seed for reproducibility

    % Group 1: capacity-related output (N×1)
    Xg{1} = Estimation(:,1);   % N×1
    
    % Group 2: life-related output (enabled for Kind = 2,3,4)
    ...
    % Group 4: expanded performance indicators (enabled for Kind = 4)
    ...
    %% ========== 2. Preprocessing: group-wise standardization ==========
    % Standardize each group independently to avoid scale dominance
    Xg_norm = cell(G,1);
    for g = 1:G
        [Xg_norm{g}, ~] = zscore_norm(Xg{g});
    end
    
    % Concatenate standardized groups into a single feature matrix for K-means
    % Manual concatenation is used to avoid cell2mat dimension issues
    Xn = [];
    for g = 1:numel(Xg_norm)
        A = Xg_norm{g};
    
        % Force 2D matrix shape
        if isvector(A)
            A = A(:); % enforce column vector (N×1)
        end
    
        % If mistakenly formed as d×N, transpose to N×d
        ...
        if size(A,1) ~= N
            error('Group %d has incorrect size: expected %d×%d, got %d×%d', ...
    ...
    % Group weights (can emphasize selected groups)
    w = [1, 1, 1, 1];  
    ...
    % Custom K-means (k-means++ init + Lloyd iterations)
    ...
    %% ========== 4. Visualization 1: PCA projection for 2D scatter ==========
    % PCA is used only for visualization (clustering is done in full space)
    ...
    title('K-means clustering (clustered in full space; PCA used only for display)');
    
    %% ========== 5. Visualization 2: cluster-center profiles per group ==========
    % Split the concatenated center matrix C back into groups and plot
    ...
    for g = 1:G
        ...
        title(sprintf('Group %d (%d dims): cluster centers', g, d));
    ...
    % Intra-cluster distance (group-weighted)
    ...
    %% ========== 7. Compute within-cluster trajectory dispersion (STD) ==========
    % For each cluster: collect per-cycle trajectories, then compute per-cycle STD
    for i = 1:K
        ...
        % Mean STD per cluster (average over cycle index m)
    ...
    % Aggregate across clusters (average across i = 1..K)
    ...
    % Clear per-cluster containers to avoid carry-over across Kind
...
% Report relative dispersion (Kind=4 vs Kind=1) for selected metrics
...
%% Summary results: normalize by Kind = 1 baseline
...
% Bar plots for relative dispersion ratios across Kind = 1..4
...
% Save summary ratios for downstream reporting
save K_Means_Result_50.mat Result