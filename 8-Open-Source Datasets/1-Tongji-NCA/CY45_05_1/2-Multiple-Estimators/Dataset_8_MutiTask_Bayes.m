...
%%% repeated leave-one-out multi-task regression workflow using a simple
%%% Bayesian linear regression (custom functions below). A single Bayesian
%%% model is trained jointly for all outputs, and predictions (Y_Test) are
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2D matrix shapes expected by the custom functions
            ...
            %% Bayesian regression hyperparameters (kept as in original)
            sigma_squared = 0.01; % variance of the observation-noise covariance
            eta_sqaured   = 0.02; % variance of the prior covariance for regression weights

            %% Train Bayesian linear regression (multi-output)
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    % Get feature dimension (M) and number of samples (N)
    M = size(p_train, 1);
    N = size(p_train, 2);

    % Construct covariance matrices (kept as in original implementation)
    big_sigma = small_sigma_squared * eye(M);
    big_omega = eta_sqaured * eye(N);

    % Posterior precision matrix and posterior mean of regression weights
...
function t_sim = my_sim_bayes(model, p_train)
    % Predict outputs using the learned regression weights
    t_sim = p_train * model;
end