...
%%% repeated leave-one-out multi-task regression workflow using decision
%%% trees (fitrtree). One regression tree is trained per output. The test
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2D matrix shapes expected by fitrtree/predict
            ...
            % Train decision trees (one regression tree per output)
            trees = cell(6, 1);
            for i = 1:6
