...
%%% repeated leave-one-out multi-task regression workflow using KNN models.
%%% One KNN regressor is trained per output (with correlation distance),
%%% and test predictions (Y_Test) are saved as MAT-files for downstream
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2D matrix shapes expected by fitcknn/predict
            ...
            %% Create KNN regressors (one per output)
            % NOTE: The original code uses fitcknn; this is kept unchanged.
            %       Correlation distance is used with NumNeighbors = 3.
