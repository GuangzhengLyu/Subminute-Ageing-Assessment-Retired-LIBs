...
%%% This script: Assemble RC-type relaxation features (Uoc, R0, R1, C1, R2, C2)
%%% and expanded health indicators, apply min–max normalization, and run a
%%% repeated leave-one-out PLSR workflow to generate prediction results.
%%% The test predictions (Y_Test) are saved as MAT-files for downstream
...
% Output columns:
% 1) Capacity-based SOH proxy (scaled Capa)
% 2) Life proxy (Life)
% 3) Energy efficiency (ERate)
% 4) Constant-current charge rate (CoChRate)
% 5) Mid-point voltage (MindVolt)
% 6) Platform discharge capacity (PlatfCapa)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2D matrix shapes expected by plsregress
            ...
            % 2) Train PLSR (automatic component count selection)
            ncomp = min(5, size(In_Train,1));  % at most 5 components
            [~, ~, ~, ~, beta] = plsregress(In_Train', Out_Train', ncomp);
            
            % 3) Predict (train and the held-out test sample)
        ...
        % Capacity (indicator 1) uses parentheses in the original script (kept)
    ...
    currentFile = sprintf('PLSR_Result_1_70_Y_Test_%d_TongjiNCA.mat',CountSV);
