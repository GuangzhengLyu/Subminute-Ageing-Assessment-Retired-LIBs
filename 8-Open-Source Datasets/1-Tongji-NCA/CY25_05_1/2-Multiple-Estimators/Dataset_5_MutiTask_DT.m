...
%%% then run repeated leave-one-out evaluation using multi-output regression
%%% trees (fitrtree) at a selected voltage setpoint. One regression tree is
%%% trained per output dimension, predictions are de-normalized back to the
%%% original scales, and test predictions are saved to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Repeated LOOCV evaluation (Regression Trees)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes are consistent for fitrtree/predict inputs
            ...
            % Train regression trees (one tree per output dimension)
            trees = cell(6, 1);
