...
%%% then run repeated leave-one-out evaluation using multi-output random
%%% forests (TreeBagger, regression mode) at a selected voltage setpoint.
%%% One random-forest regressor is trained per output dimension, predictions
%%% are de-normalized back to the original scales, and test predictions are
%%% saved to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Repeated LOOCV evaluation (Random Forest, TreeBagger)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes are consistent for TreeBagger/predict inputs
            ...
            %% Random Forest (TreeBagger, regression)
            ...
            % Predict on training set and the held-out test sample (one model per output)
