...
%%% then run repeated leave-one-out evaluation using XGBoost regressors at a
%%% selected voltage setpoint. One XGBoost model is trained per output
%%% dimension via the provided MATLAB XGBoost wrapper functions. Predictions
...
%% Repeated LOOCV evaluation (XGBoost)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes are consistent for the XGBoost wrappers
            ...
            %% XGBoost
            % Train one model per output dimension using the provided wrappers
            num_trees = 100;
            params.eta = 0.1;
