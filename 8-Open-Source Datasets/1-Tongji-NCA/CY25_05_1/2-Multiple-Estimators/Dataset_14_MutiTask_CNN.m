...
%%% This script: Assemble multi-setpoint relaxation-feature tensors and
%%% normalized outputs, then run repeated leave-one-out evaluation using a
%%% compact CNN regressor (2D convolution + fully connected layers) built
%%% with MATLAB Deep Learning Toolbox (trainNetwork). Models are trained on
%%% normalized inputs/outputs and predictions are de-normalized back to the
%%% original scales. Test-set predictions are saved to a MAT-file for
%%% downstream comparison across models.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% samples (row) and voltage setpoints (column). This script uses all
% available setpoints (NumST dimension) when assembling CNN inputs.
Feature(:,:,1) = Uoc;
Feature(:,:,2) = R0;
Feature(:,:,3) = R1;
Feature(:,:,4) = C1;
...
%% Repeated LOOCV evaluation (CNN via trainNetwork)
% For the selected setpoint index (CountSV), normalize input features using
% setpoint-specific bounds, then assemble 4-D CNN inputs that stack all
% setpoints as the first spatial dimension.
for CountSV = 14:-1:14
    ...
    % Min-max normalize the 6 relaxation features across samples and setpoints
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Assemble CNN test input (4-D): [NumST, 6, 1, 1]
            % NumST corresponds to the number of setpoints in Temp_F(:,:,NumST).
            for NumST = 1:size(Temp_F,3)
                In_Test(NumST,:,1,1) = Temp_F(IndexData,:,NumST);
            end
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';

            % Remove held-out sample from the training pool
            Temp_F(IndexData,:) = [];
            Temp_O(IndexData,:) = [];

            % Assemble CNN training input (4-D): [NumST, 6, 1, NumID]
            for NumST = 1:size(Temp_F,3)
            ...
            %% CNN regressor definition (kept as in original script)
            % Input size: [NumST, 6, 1], where NumST is treated as the first
            % spatial dimension and the 6 features are treated as the second.
            layers = [
            ...
            % Train network on the current LOOCV split
            net = trainNetwork(In_Train,Out_Train',layers,options);

            % Predict on train and held-out test sample
