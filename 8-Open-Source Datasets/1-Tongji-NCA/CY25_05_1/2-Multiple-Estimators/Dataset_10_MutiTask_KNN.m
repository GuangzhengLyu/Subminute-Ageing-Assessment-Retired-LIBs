...
%%% then run repeated leave-one-out evaluation using a KNN regressor
%%% (implemented via fitcknn with correlation distance). Models are trained
%%% on normalized inputs/outputs and predictions are de-normalized back to
%%% the original scales. Test-set predictions are saved to a MAT-file for
...
%% Repeated LOOCV evaluation (KNN via fitcknn)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Create and train KNN models (one model per output dimension)
            % Note: fitcknn is used here as provided; prediction returns class labels
            % if Out_Train is treated as categorical. The original implementation is
            % preserved to avoid modifying executable logic.
            ...
            % Prediction on training set and held-out test sample
