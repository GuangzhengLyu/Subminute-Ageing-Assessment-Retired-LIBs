...
%%% then run repeated leave-one-out evaluation using a Bayesian-regularized
%%% neural network (trainbr). The network is trained on normalized
%%% inputs/outputs and predictions are de-normalized back to the original
%%% scales. Test-set predictions are saved to a MAT-file for downstream
...
%% Repeated LOOCV evaluation (Bayesian-regularized neural network)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes match Neural Network Toolbox conventions
            ...
            net.trainParam.showWindow = false; % Disable GUI for batch runs
            net.trainParam.epochs = 20;        % Maximum training epochs
            
            % 3. Train network on normalized training data
            net = train(net, In_Train, Out_Train);

            % Prediction (training set and held-out test sample)
