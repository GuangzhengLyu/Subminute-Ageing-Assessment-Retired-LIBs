...
%%% then run repeated leave-one-out evaluation using multi-output SVR
%%% (LIBSVM SVR with RBF kernel) at a selected voltage setpoint. For each
%%% output dimension, the script performs grid search for (C, gamma) using
%%% cross-validation, fits an SVR model, predicts on train/test splits, and
%%% saves test predictions to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% SVR library functions (kept as in original script)
addpath('./4-SVR');
...
% used during SVR fitting/prediction below.
Capa = Capa/3.5;
Life = Life;
ERate = ERate/0.92;
...
%% Repeated LOOCV evaluation (SVR, RBF kernel)
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes are consistent for LIBSVM inputs
            ...
            %% Model 1
            % 1) Search best C / gamma by grid search with v-fold CV
            ...
            for i = 1:m
                for j = 1:n
                    % -t 2: RBF kernel; -v: return CV error; -s 3: epsilon-SVR
            ...
            % 2) Train SVR using best C / gamma
