...
%%% then run repeated leave-one-out evaluation using a shallow DNN regressor
%%% implemented with MATLAB Deep Learning Toolbox (trainNetwork). Models are
%%% trained on normalized inputs/outputs and predictions are de-normalized
%%% back to the original scales. Test-set predictions are saved to a MAT-file
%%% for downstream comparison across models.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Repeated LOOCV evaluation (DNN via trainNetwork)
...
for CountSV = 14:-1:14
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Ensure 2-D shapes match downstream routines
            ...
            %% DNN regressor definition (kept as in original script)
            % Note: The architecture is a fully-connected network with 6 inputs
            % and 6 outputs. Training uses Adam with piecewise learning-rate drop.
            layers = [
