...
%%% Dataset: 1-Tongji-NCA (TongjiNCA25)
%%% This script: Assemble relaxation-feature tensors and normalized outputs,
%%% then run repeated leave-one-out PLSR at a selected voltage setpoint and
%%% save the repeated test predictions to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Note: This is a unified engineering scaling, not the final [0,1] min-max
% normalization used by the downstream PLSR evaluation.
Capa = Capa/3.5;
Life = Life;
ERate = ERate/0.92;
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Predict on training and held-out test sample (normalized output space)
    ...
    currentFile = sprintf('PLSR_Result_1_70_Y_Test_%d_TongjiNCA25.mat',CountSV);
