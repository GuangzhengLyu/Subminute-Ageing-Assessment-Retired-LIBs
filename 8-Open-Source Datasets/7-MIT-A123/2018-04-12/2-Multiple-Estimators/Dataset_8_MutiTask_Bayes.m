...
%%% relaxation features, and run repeated leave-one-out Bayesian linear
%%% regression to generate and save results.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Bayesian linear regression (ridge-like closed-form posterior mean)
            % Hyperparameters (kept as in original script)
            sigma_squared = 0.01; % variance of output covariance
            eta_sqaured   = 0.02; % variance of beta (prior covariance)

            % Train model (posterior mean of regression coefficients)
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    ...
    % Covariance settings (kept as simple diagonal forms)
    big_sigma = small_sigma_squared * eye(M);
    big_omega = eta_sqaured * eye(N);

    % Closed-form posterior mean of regression coefficients
...
function t_sim = my_sim_bayes(model, p_train)
    % Predict outputs using the learned regression coefficients
    t_sim = p_train * model;
end