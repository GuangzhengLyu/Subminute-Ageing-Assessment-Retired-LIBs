...
%%% This script: Extract a relaxation-voltage segment at a fixed setpoint,
%%% fit a double-exponential relaxation model, and assemble RC-type
%%% relaxation parameters (Uoc, R0, R1, C1, R2, C2) for downstream analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for IndexData = 1:length(OneCycle)
   ...
   %% Relaxation-voltage extraction
    ...
    if ~ismember(IndexData,[])
        ...
        % Identify candidate relaxation points: zero current and high voltage region
        Index1 = find(OneCycle(IndexData).CurrentA == 0);
        Index2 = find(OneCycle(IndexData).VoltageV >= 3.5);
        IndexRX = intersect(Index1,Index2);

        % Keep only the last contiguous relaxation block (remove earlier discontinuities)
        TempIR = IndexRX;
        ...
        % Store relaxation voltage trace (note the "-1" offset is kept as in original)
...
for i = 1:-1:1
    ...
    % The following block is kept (commented) as in the original script for optional use
    ...
    figure(1),hold on,plot(Capa,Uoc(:,indexRX),'o')
    figure(2),hold on,plot(Capa,R0(:,indexRX),'d')
    figure(3),hold on,plot(Capa,R1(:,indexRX),'<')
    figure(4),hold on,plot(Capa,C1(:,indexRX),'>')
    figure(5),hold on,plot(Capa,R2(:,indexRX),'*')
    figure(6),hold on,plot(Capa,C2(:,indexRX),'+')
end

% Save assembled relaxation-feature matrices for downstream modelling
save Feature_ALL_MITA123_3.mat Uoc R0 R1 C1 R2 C2