...
%%% features, train per-task Gaussian Process Regression (GPR) models under
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out GPR (one model per task/output)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data in the shape expected by fitrgp/predict
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            % Train per-output Gaussian Process Regression models
            % (comment kept consistent with original script structure)
