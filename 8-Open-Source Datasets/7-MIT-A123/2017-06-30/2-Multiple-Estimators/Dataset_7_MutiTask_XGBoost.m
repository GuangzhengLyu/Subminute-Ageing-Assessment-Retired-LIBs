...
%%% features, train per-task XGBoost regression models under repeated
%%% leave-one-out splits, visualize train/test scatter results, and save
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out XGBoost (one model per task/output)
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data for xgboost_train/xgboost_test
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            %% XGBoost configuration (kept as in original script)
            num_trees = 100;
            params.eta = 0.1;
            ...
            % Train one XGBoost model per output (multi-task via per-task models)
