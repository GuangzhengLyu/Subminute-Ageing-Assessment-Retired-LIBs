...
%%% features, train per-task multiple linear regression (MLR) models under
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out MLR (one model per task/output)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data in the shape expected by fitlm/predict
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            % Train separate linear models for each output (multi-task via per-task models)
            ...
            % Predict on training set (for scatter visualization)
            ...
            % Predict the left-out test sample
