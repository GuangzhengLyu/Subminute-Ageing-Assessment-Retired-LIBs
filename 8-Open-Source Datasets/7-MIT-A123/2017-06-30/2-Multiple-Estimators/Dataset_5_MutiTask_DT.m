...
%%% features, train per-task regression decision trees (one tree per output)
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out decision trees (one tree per task/output)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data in the shape expected by fitrtree/predict
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            % 2. Train regression decision trees (one tree per output)
            trees = cell(6, 1);
