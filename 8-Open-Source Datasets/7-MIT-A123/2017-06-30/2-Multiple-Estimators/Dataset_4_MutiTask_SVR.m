...
%%% features, train per-task Support Vector Regression (SVR, RBF kernel)
%%% models with grid search for (C, gamma), visualize train/test scatter
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out SVR (one model per task/output)
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data for libsvm format
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1']; % 2: RBF; -v: CV error; -s 3: SVR
                    ...
                    if abs(cg(i,j) - error) <= eps && bestc > 2^c(i,j) % prioritize smaller C on tie
                        error = cg(i,j);
                        bestc = 2^c(i,j);
                        bestg = 2^g(i,j);
                    end
