...
%%% features, train a Bayesian linear regression model (shared across the six
%%% outputs) under repeated leave-one-out splits, visualize train/test scatter
%%% results, and save predicted test outputs to MAT-files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out Bayesian regression (shared multi-output model)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Set Bayesian regression hyperparameters
            sigma_squared = 0.01; % output covariance variance
            eta_sqaured   = 0.02; % beta covariance variance
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    ...
    % Prior/likelihood covariance terms (kept as in original implementation)
...
function t_sim = my_sim_bayes(model, p_train)
    % Generate predictions from the learned linear model
    t_sim = p_train * model;
end