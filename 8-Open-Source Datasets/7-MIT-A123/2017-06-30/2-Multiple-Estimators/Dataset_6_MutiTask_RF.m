...
%%% features, train per-task Random Forest models (TreeBagger regression)
%%% under repeated leave-one-out splits, visualize train/test scatter results,
%%% and save predicted test outputs to MAT-files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14
    ...
    %% Repeated leave-one-out Random Forest (TreeBagger regression)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data in the shape expected by TreeBagger/predict
            In_Train = Temp_F';
            Out_Train = Temp_O';
            ...
            %% Random Forest configuration (kept as in original script)
            trees = 100;                                      % number of trees
            leaf  = 5;                                        % minimum leaf size
            OOBPrediction = 'on';                             % enable OOB error estimation
            OOBPredictorImportance = 'on';                    % compute predictor importance

            Method = 'regression';                            % regression mode

            % Train one RF model per output (multi-task via per-task ensembles)
