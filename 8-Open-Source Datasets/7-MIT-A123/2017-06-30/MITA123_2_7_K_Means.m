...
%%% Dataset: 7-MIT-A123
%%% This script: Load MITA123_2 data, relaxation features, and PLSR-based
%%% estimations; perform grouped-weighted K-means clustering under different
%%% feature-group settings; compute within-cluster dispersion (STD-based)
%%% trajectories for multiple engineering indicators; summarize and save
%%% normalized consistency results.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Add grouped-weighted K-means implementation to path
addpath('./G-K-means')

%% ========== 1. Load grouped data ==========
load('./OneCycle_MITA123_2.mat')  
load('./Feature_ALL_MITA123_2.mat')
load('./PLSR_Result_1_80_Y_Test_14_MITA123_2.mat')
...
%% Unify as health-indicator groups
% Xg is a cell array where each cell stores one feature group (N x d_g)
Xg = cell(4,1);

for Kind = 1:4
    % Fix random seed for reproducibility (k-means++ init / multiple restarts)
    rng(42);

    % Group 1: capacity-related estimate (always included)
    Xg{1} = Estimation(:,1);   % N×1

    % Group 2: life-related estimate (enabled for Kind = 2,3,4)
    Xg{2} = [Estimation(:,2)];
    ...
    % Group 4: expanded health-indicator estimates (enabled for Kind = 4 only)
    ...
    % Rebuild full feature matrix for K-means (manual concatenation)
    % Note: This avoids cell2mat failures when shape inconsistencies occur
    Xn = [];
    ...
    % Set per-group weights to emphasize certain groups if needed
    w = [1, 1, 1, 1];
    ...
    fprintf('\nClustering done: best SSE = %.4f, iter = %d\n', info.bestSSE, info.bestIter);

    %% ========== 4. Visualization 1: PCA 2D scatter ==========
    % PCA is used only for visualization; clustering is performed in full space
    ...
    % Split centers C back into groups and plot the profile of each group
    figure('Name','Cluster-center profiles for each feature group');
    ...
    %% ========== 7. Compute within-cluster dispersion trajectories ==========
    % For each cluster: compute per-cycle STD across cells, then average STD
    for i = 1:K
    ...
    % Aggregate mean STD across clusters for this Kind setting
    ...
    % Clear intermediate variables to avoid leakage across Kind loops
...
% Ratio summary (Kind=4 vs Kind=1) for quick comparison
...
%% Normalize results to Kind=1 baseline and visualize
...
% Save normalized consistency results for downstream comparison
save K_Means_Result_2.mat Result