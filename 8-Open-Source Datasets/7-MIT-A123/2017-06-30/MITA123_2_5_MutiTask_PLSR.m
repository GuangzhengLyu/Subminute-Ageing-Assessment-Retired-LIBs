...
%%% Dataset: 7-MIT-A123
%%% This script: Load MITA123_2 single-cycle data and fitted relaxation
%%% features, construct and normalize target health indicators, normalize
%%% input features, run repeated leave-one-out PLSR, visualize results, and
%%% save the predicted test outputs to MAT-files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Unify as normalized health indicators (dataset-specific references)
...
for CountSV = 14:-1:14
    ...
    % For each repetition: leave one sample out, fit PLSR on remaining data,
    % predict on the left-out sample, and visualize train/test results.
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange training data in the shape expected by plsregress
            In_Train = Temp_F';
            Out_Train = Temp_O';
    ...
    currentFile = sprintf('PLSR_Result_1_80_Y_Test_%d_MITA123_2.mat',CountSV);
