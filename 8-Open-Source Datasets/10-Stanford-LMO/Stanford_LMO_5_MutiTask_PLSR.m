...
%%% This script: Build normalized health-indicator targets and relaxation-
%%% feature inputs, then run repeated leave-one-out PLSR to predict multiple
%%% ageing indicators. Predicted test outputs (Y_Test) are saved per setting
%%% to MAT-files for downstream evaluation and result aggregation.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14
    ...
    for CountRP = 1:100
        ...
        for IndexData = 1:length(Feature_Nor)
            ...
            % 2. Train PLSR (automatic selection of component count)
            ncomp = min(5, size(In_Train,1));  % up to 5 components
            [~, ~, ~, ~, beta] = plsregress(In_Train', Out_Train', ncomp);

            % 3. Predict train/test outputs (normalized space)
    ...
    currentFile = sprintf('PLSR_Result_1_50_Y_Test_%d_Stanford_LMO.mat',CountSV);
