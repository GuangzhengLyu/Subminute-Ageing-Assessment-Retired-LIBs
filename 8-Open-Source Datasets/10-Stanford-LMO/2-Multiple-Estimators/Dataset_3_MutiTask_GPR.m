...
%%% Gaussian process regression (GPR) to predict six ageing indicators.
%%% Predicted test outputs (Y_Test) are saved to MAT-files for downstream
...
% Note: The script keeps both normalization blocks as in the original file.
% The second block overwrites the first one; structure is preserved.
Capa = Capa/3.5;
Life = Life;
ERate = ERate/0.92;
...
%% Convert to unified health indicators (final scaling used in this script)
Capa = Capa/32.5;
Life = Life;
ERate = ERate/0.99;
CoChRate = CoChRate/1;
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        ...
        for IndexData = 1:length(Feature_Nor)
            ...
            % Train GPR models for each output (independent single-output regressors)
