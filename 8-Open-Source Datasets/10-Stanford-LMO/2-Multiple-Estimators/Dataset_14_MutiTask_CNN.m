...
%%% convolutional neural network (CNN) regression (trainNetwork) to predict
%%% six ageing indicators. Predicted test outputs (Y_Test) are saved to
%%% MAT-files for downstream evaluation and result aggregation.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        ...
        for IndexData = 1:length(Feature_Nor)
            ...
            % Arrange held-out test sample as an "image-like" tensor
            for NumST = 1:size(Temp_F,3)
                In_Test(NumST,:,1,1) = Temp_F(IndexData,:,NumST);
            end

            % Hold-out target (kept original storage layout)
            Out_Test(CountRP,:,IndexData) = Temp_O(IndexData,:)';
        
            % Remove held-out sample from training set
            Temp_F(IndexData,:) = [];
            Temp_O(IndexData,:) = [];
        
            % Arrange training set as 4-D array: H×W×C×N
            for NumST = 1:size(Temp_F,3)
            ...
            % Targets: 6×N (kept original transpose usage in trainNetwork call)
            Out_Train = Temp_O';
            
            %% Multi-task CNN regression (Deep Learning Toolbox)
            layers = [
            ...
            % Training settings (kept unchanged)
