...
%%% leave-one-out multi-task regression using GPR (fitrgp) for each output.
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Multi-task GPR training (one GPR model per output)
            % Note: the model name comment is kept as original (even if mismatched).
