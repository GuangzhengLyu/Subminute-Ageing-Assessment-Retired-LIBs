...
%%% leave-one-out multi-task regression using K-nearest neighbors (KNN).
%%% Separate KNN models are trained for each output dimension. Prediction
%%% results (Y_Test) are saved to MAT-files for downstream analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Create and train KNN models (one model per output)
            % Note: fitcknn is used as in the original script (kept unchanged).
            % With numeric responses, predict() returns class labels; the script
            % intentionally keeps this behavior as-is for reproducibility.
