...
%%% leave-one-out multi-task regression using SVR (LIBSVM). For each output,
%%% an RBF-kernel SVR model is trained with a grid search over (C, gamma)
%%% using cross-validation. Prediction results (Y_Test) are saved to MAT-files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Add SVR toolbox (LIBSVM functions: svmtrain/svmpredict)
% SVR library functions
addpath('./4-SVR');
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1'];%2 Gaussian (RBF) kernel; -v returns CV error; -s 3 for SVR
                    ...
                    if abs(cg(i,j) - error) <= eps && bestc > 2^c(i,j)% prioritize smaller C under ties
                        error = cg(i,j);
                        bestc = 2^c(i,j);
                        bestg = 2^g(i,j);
                    end
            ...
            % 2. Train SVR model with best hyperparameters
