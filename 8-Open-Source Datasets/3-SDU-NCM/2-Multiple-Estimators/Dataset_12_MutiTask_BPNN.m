...
%%% leave-one-out multi-task regression using a backpropagation neural
%%% network (BPNN). Predictions (Y_Test) are saved to MAT-files for
%%% downstream accuracy and consistency analyses.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Define transfer functions and training algorithm (kept as original)
            transform_func = {'tansig','purelin'};
            train_func = 'trainlm';

            % Create a feedforward network with the specified layer sizes
            net = newff(In_Train,Out_Train,[6,20,10,6],transform_func,train_func);

            % Training hyperparameters (kept as original)
            net.trainParam.epochs = 100000000;
            net.trainParam.lr = 0.001;
            net.trainParam.goal = 0.0001;

            % Train the network on the current leave-one-out training set
            net = train(net,In_Train,Out_Train);

            % Predict on training set and the left-out test sample
