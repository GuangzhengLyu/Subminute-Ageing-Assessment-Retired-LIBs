...
for IndexData = 1:length(OneCycle)
    ...
    % Define life as the first cycle where discharge capacity drops below 1.2 Ah
    % If the threshold is not reached, use the full trajectory length
    if (min(OneCycle(IndexData).Cycle.DiscCapaAh) <= 1.2)
        Life(CountData,1) = min(find(OneCycle(IndexData).Cycle.DiscCapaAh < 1.2));
    else
   ...
   %% Relaxation-voltage extraction
    ...
    if ~ismember(IndexData,[])
        ...
        % LengthDrop: number of initial points to discard in the relaxation segment
        % (kept as in original script; do not change)
        LengthDrop = 5; % LengthDrop = 0;
        LengthCops = 0;

        % Fixed terminal-voltage setpoint for relaxation extraction/fitting
        Vset = 4.2;
        ...
        % Store relaxation voltage trace (with the original indexing rule preserved)
      ...
      %% Health-feature construction
        ...
        if ismember(IndexData, [5,8,52,64,71])%ismember(IndexData, [5,8,52,64])
            Bias = 0;
...
save Feature_ALL_SDU_NCM.mat Uoc R0 R1 C1 R2 C2