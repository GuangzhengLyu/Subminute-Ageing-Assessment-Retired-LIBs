...
%%% leave-one-out Gaussian Process Regression (GPR) using RC relaxation
%%% features as inputs. One GPR model is fitted per target. Predictions are
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out Gaussian Process Regression (GPR)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Train one GPR model per target (multi-task via separate heads)
