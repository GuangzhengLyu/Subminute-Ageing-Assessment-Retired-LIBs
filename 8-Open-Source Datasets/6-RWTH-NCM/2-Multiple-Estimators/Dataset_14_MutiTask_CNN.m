...
%%% Dataset: 6-RWTH-NCM
%%% This script: Build multi-task targets and run repeated leave-one-out
%%% convolutional neural network (CNN) regression. Inputs are relaxation-
%%% feature "images" assembled from the Feature tensor. Outputs include
%%% capacity, life, and expanded performance indicators. Predictions are
%%% clamped, de-normalized, visualized, and saved to MAT files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14
    ...
    % Clamp normalized features/targets
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Build test input as a 4-D array compatible with imageInputLayer:
            % [H, W, C, N] = [size(In_Train,1), 6, 1, 1]
            ...
            % Build training input tensor as 4-D array: [H, W, C, N]
            for NumST = 1:size(Temp_F,3)
            ...
            % CNN architecture (treat stacked feature maps as an "image")
            layers = [
