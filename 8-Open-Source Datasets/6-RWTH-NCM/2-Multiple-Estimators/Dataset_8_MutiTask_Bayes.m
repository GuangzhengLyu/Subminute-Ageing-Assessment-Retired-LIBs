...
%%% leave-one-out Bayesian linear regression using RC relaxation features as
%%% inputs. A single Bayesian regression model is trained to jointly predict
%%% all targets. Predictions are de-normalized for visualization and saved
%%% to MAT files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out Bayesian regression (multi-output)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            eta_sqaured   = 0.02; % Variance of prior covariance for regression weights
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    % Get the number of features and samples from the input matrix
    M = size(p_train, 1);
    N = size(p_train, 2);
    %
    big_sigma = small_sigma_squared * eye(M);
    big_omega = eta_sqaured * eye(N);
    % Solve for the regression coefficients (posterior mean)
...
function t_sim = my_sim_bayes(model, p_train)
    % Compute predictions using the learned regression coefficients
    t_sim = p_train * model;
end