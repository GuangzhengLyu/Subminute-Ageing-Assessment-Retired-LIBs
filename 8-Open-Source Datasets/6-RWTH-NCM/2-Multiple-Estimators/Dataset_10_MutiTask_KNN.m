...
%%% leave-one-out KNN regression using RC relaxation features as inputs.
%%% One KNN model is trained per target. Predictions are de-normalized for
...
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out KNN regression (multi-task via separate heads)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Create and train KNN models (one per target)
