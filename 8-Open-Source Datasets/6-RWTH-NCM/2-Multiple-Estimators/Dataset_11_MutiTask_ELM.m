...
%%% leave-one-out extreme learning machine (ELM) regression using RC
%%% relaxation features as inputs. A single multi-output ELM is trained to
%%% jointly predict all targets. Predictions are de-normalized for
...
% Add ELM toolbox functions to MATLAB path
addpath('./11-ELM');
...
% Main loop (kept as in the original script)
for CountSV = 14:-1:14 
    ...
    %% Repeated leave-one-out ELM regression (multi-output)
    for CountRP = 1:100
