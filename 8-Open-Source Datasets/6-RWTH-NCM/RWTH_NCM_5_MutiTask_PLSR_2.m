...
%%% Dataset: 6-RWTH-NCM
%%% This script: Build per-cell target outputs (capacity, life, and expanded
%%% performance indicators), normalize features/targets, and run repeated
%%% leave-one-out PLSR to predict multiple health indicators from fitted
%%% relaxation (RC-type) features. The prediction results are saved to MAT
%%% files for downstream evaluation/visualization.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Sample construction
% Build per-sample targets: life proxy, original capacity, and expanded indicators
CountData = 0;
for IndexData = 1:length(OneCycle)
    CountData = CountData+1;

    % Define "Life" using a trajectory rule based on discharge capacity
    % If the discharge capacity drops to <= 1 Ah at any point, take the last cycle
    % index where capacity is still > 0.5 Ah; otherwise use the full length.
    if (min(OneCycle(IndexData).Cycle.DiscCapaAh) <= 1)
        Life(CountData,1) = max(find(OneCycle(IndexData).Cycle.DiscCapaAh > 0.5));
    else
...
for CountSV = 14:-1:14
    ...
    % For each repetition (CountRP), perform leave-one-out training/testing
    % and store predictions Y_Test for all outputs and all samples.
    for CountRP = 1:100
        ...
        % Train: Out_Train vs Y_Train
        % Test:  Out_Test vs Y_Test (per repetition)
        for IndOTrain = 1:6
    ...
    currentFile = sprintf('PLSR_Result_1_50_Y_Test_%d_RWTH_NCM.mat',CountSV);
