...
%%% relaxation features (Uoc, R0, R1, C1, R2, C2) for downstream ageing
...
for IndexData = 1:length(OneCycle)
   ...
   %% Relaxation-voltage extraction
    ...
    if ~ismember(IndexData,[])
        ...
        % Keep only the final continuous relaxation segment (drop earlier segments if gaps exist)
        TempIR = IndexRX;
        ...
        % Store relaxation voltage trace (note: include one sample before the first point)
...
save Feature_ALL_MITA123_4.mat Uoc R0 R1 C1 R2 C2