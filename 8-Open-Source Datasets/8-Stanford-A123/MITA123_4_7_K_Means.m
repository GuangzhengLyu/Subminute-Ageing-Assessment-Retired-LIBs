...
%%% This script: Run grouped/weighted K-means clustering on health estimates
%%% and relaxation-parameter features, visualize clusters, and evaluate
%%% within-cluster consistency via trajectory-wise standard deviation metrics.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
load('./PLSR_Result_1_80_Y_Test_14_MITA123_4.mat')

% Use the mean prediction across repeated runs as the point estimate
Estimation = squeeze(mean(Y_Test,1))';

% Assemble relaxation-feature tensor (samples × segments × features)
Feature(:,:,1) = Uoc;
Feature(:,:,2) = R0;
Feature(:,:,3) = R1;
Feature(:,:,4) = C1;
...
%% Unify as health indicators (group definition)
% Xg is a cell array of feature groups. Depending on Kind, some groups are
% disabled by setting them to zeros (structure preserved for fair comparison).
Xg = cell(4,1);
...
for Kind = 1:4
    ...
    % Group 3: relaxation-parameter features (enabled for Kind = 3/4)
    ...
    % Standardize each group independently (z-score), then concatenate.
    Xg_norm = cell(G,1);
    for g = 1:G
        [Xg_norm{g}, ~] = zscore_norm(Xg{g});
    end
    
    % Concatenate to the full feature matrix for K-means
    % Manual concatenation is used to avoid dimension issues with cell2mat.
    Xn = [];
    for g = 1:numel(Xg_norm)
        A = Xg_norm{g};
    
        % Enforce 2D matrix format
        if isvector(A)
            A = A(:); % Force column vector (N×1)
        end
    
        % If accidentally transposed as d×N, transpose back to N×d
        ...
        if size(A,1) ~= N
            error('Group %d has wrong dimensions: expected %d×%d, got %d×%d', ...
    ...
    % Set group weights to reflect group importance (equal weights by default)
    w = [1, 1, 1, 1];  
    ...
    % Custom K-means: k-means++ initialization + Lloyd updates
    ...
    % Split the learned centers C back into each group and plot per-group profiles.
    ...
    % Simple within-cluster distance summary (group-weighted)
    ...
    %% ========== 7. Within-cluster trajectory dispersion metrics ==========
    % For each cluster, compute the cycle-wise standard deviation of several
    % engineering trajectories, then average across cycles and clusters.
    for i = 1:K
    ...
    % Aggregate dispersion across clusters for the current Kind setting
    ...
    % Clear per-loop variables to avoid unintended carry-over
...
% Relative improvement from Kind=1 to Kind=4 (ratio form)
...
%% Summarize results as ratios normalized by Kind = 1
...
% Bar plots for each dispersion metric ratio (Kind = 1 baseline)
