...
%%% inputs, apply min-max normalization, and run repeated leave-one-out SVR
%%% (LIBSVM, RBF kernel with grid-search hyperparameters) to generate
%%% predictions saved as MAT-files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Add SVR library functions (LIBSVM wrapper)
addpath('./4-SVR');
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            %% SVR model training (RBF kernel, grid-search for hyperparameters)
            % Note: Hyperparameter search is performed independently per output.
            % Grid search: c = 2^cGrid, g = 2^gGrid, with v-fold CV in LIBSVM.
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1'];% 2: RBF kernel; -v: CV; -s 3: SVR
