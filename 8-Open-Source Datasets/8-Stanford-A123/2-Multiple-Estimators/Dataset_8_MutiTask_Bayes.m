...
%%% parameter features, apply min–max normalization, and run repeated
%%% leave-one-out Bayesian linear regression to generate predictions.
%%% Prediction results are saved as MAT-files for model comparison.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Set Bayesian regression hyperparameters (kept as in the original script)
            sigma_squared = 0.01; % Variance of output noise covariance
            eta_sqaured   = 0.02; % Variance of prior covariance for beta
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    ...
    % Construct covariance matrices (kept as in the original script)
    big_sigma = small_sigma_squared * eye(M);
    big_omega = eta_sqaured * eye(N);

    % Solve for posterior mean of regression coefficients
...
function t_sim = my_sim_bayes(model, p_train)
    % Predict output using the learned linear model coefficients
    t_sim = p_train * model;
end