...
%%% leave-one-out backpropagation neural network (BPNN) regression.
%%% Predictions are saved as MAT-files for downstream benchmarking.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Note: newff/train/sim are used as in the original script.
            transform_func = {'tansig','purelin'};
            train_func = 'trainlm';
            net = newff(In_Train,Out_Train,[6,20,10,6],transform_func,train_func);

            % Training hyperparameters (kept as in original script)
            ...
            Y_Train = sim(net,In_Train);
            Y_Test(CountRP,:,IndexData) = sim(net,In_Test(:,IndexData));
    ...
    currentFile = sprintf('Result_12_Y_Test_%d.mat',CountSV);
