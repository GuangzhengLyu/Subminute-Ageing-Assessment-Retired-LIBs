...
%%% Dataset: 9-SCU1-A123
%%% This script: Assemble normalized inputs (relaxation features) and outputs
%%% (capacity/life plus extended health indicators), then run repeated
%%% leave-one-out PLSR to predict health indicators from relaxation features.
%%% The prediction results (Y_Test) are saved for downstream analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for IndexData = 1:length(OneCycle)
    ...
    % Define life as the first cycle where discharge capacity drops below 0.96 Ah
    % If the threshold is not reached, use the full trajectory length
    if (min(OneCycle(IndexData).Cycle.DiscCapaAh) <= 0.96)
        Life(CountData,1) = min(find(OneCycle(IndexData).Cycle.DiscCapaAh < 0.96));
    else
    ...
    % Extended health indicators (kept as in original script)
...
%% Unify health indicators (manual scaling to comparable ranges)
Capa = Capa/1.2;
Life = Life;
ERate = ERate/81;
...
for CountSV = 14:-1:14
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            ncomp = min(3, size(In_Train,1));  % at most 5 components
    ...
    currentFile = sprintf('PLSR_Result_1_80_Y_Test_%d_SCUA123.mat',CountSV);
