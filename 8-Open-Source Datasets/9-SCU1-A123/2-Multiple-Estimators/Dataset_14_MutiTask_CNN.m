...
%%% Dataset: 9-SCU1-A123
%%% This script: Build multi-task health outputs and normalize them to [0,1].
%%% Then normalize relaxation-feature inputs and run repeated leave-one-out
%%% CNN regression (imageInputLayer + conv2d) to predict six outputs from the
%%% normalized relaxation-feature map. The per-repeat leave-one-out
%%% predictions (Y_Test) are saved to a MAT-file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Build test input as a 4-D image tensor (H×W×C×N)
            for NumST = 1:size(Temp_F,3)
                In_Test(NumST,:,1,1) = Temp_F(IndexData,:,NumST);
            end

            % Test output label (6×1)
            ...
            % Build training input as a 4-D image tensor (H×W×C×N)
            for NumST = 1:size(Temp_F,3)
            ...
            % Training outputs: 6×N (transpose used below for trainNetwork)
            Out_Train = Temp_O';

            % Define a lightweight CNN regressor for the feature map
            layers = [
