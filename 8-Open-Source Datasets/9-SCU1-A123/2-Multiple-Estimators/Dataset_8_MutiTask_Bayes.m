...
%%% leave-one-out multi-task Bayesian linear regression to predict the six
%%% outputs from the normalized relaxation features.
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            sigma_squared = 0.01; % variance of the output covariance
            eta_sqaured   = 0.02; % variance of the coefficient (beta) covariance
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    % Get the number of features and samples in the input matrix
    M = size(p_train, 1);
    N = size(p_train, 2);

    % Construct covariance/prior matrices (kept as in original script)
    big_sigma = small_sigma_squared * eye(M);
    big_omega = eta_sqaured * eye(N);

    % Posterior mean estimate of regression coefficients
...
function t_sim = my_sim_bayes(model, p_train)
    % Compute predictions using the fitted linear model
    t_sim = p_train * model;
end