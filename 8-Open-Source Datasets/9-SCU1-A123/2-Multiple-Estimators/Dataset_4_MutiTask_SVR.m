...
%%% leave-one-out multi-task SVR (LIBSVM, RBF kernel) to predict the six
%%% outputs from the normalized relaxation features. SVR hyperparameters
%%% (C, gamma) are selected by grid search with cross-validation.
%%% Predictions (Y_Test) are saved to file.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Model 1
            % 1) Grid search for the best C and gamma
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1'];%2 Gaussian kernel; -v returns CV error; -s 3 is epsilon-SVR
            ...
            cmd = [' -t 2',' -c ',num2str(bestc),' -g ',num2str(bestg),' -s 3 -p 0.01'];% use best C/gamma as parameters
            ...
            % V. SVR prediction (train and the held-out test sample)
