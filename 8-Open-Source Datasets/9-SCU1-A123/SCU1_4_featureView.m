...
%%% Dataset: 9-SCU1-A123
%%% This script: Extract a relaxation-voltage segment (Step == 2) from each
%%% sample, fit a double-exponential relaxation model, and assemble RC-type
...
for IndexData = 1:length(OneCycle)
   ...
   %% Relaxation-voltage extraction
    ...
    if ~ismember(IndexData,[])
        ...
        % Drop initial points in the relaxation segment (e.g., transient region)
        LengthDrop = 10;

        % Time-index offset (kept for compatibility with multi-segment extension)
        LengthCops = 0;

        % Fixed terminal-voltage setpoint for relaxation extraction
        Vset = 3.6;

        % Identify relaxation segment by step label (Steps == 2)
        IndexRX = find(OneCycle(IndexData).Steps == 2);

        % Store relaxation voltage trace (drop initial points via LengthDrop)
...
save Feature_ALL_A123.mat Uoc R0 R1 C1 R2 C2