...
%%% run repeated leave-one-out cross-validation using multi-output Bayesian
%%% linear regression, visualize train/test predictions, and save Y_Test for
%%% downstream evaluation. Helper functions for Bayesian regression and
%%% prediction are defined at the end of this script.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            sigma_squared = 0.01; % output-noise covariance variance
            eta_sqaured   = 0.02; % prior covariance variance for regression coefficients

            %% Train Bayesian linear regression model (multi-output)
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    ...
    % Build covariance matrices (kept as in original script)
