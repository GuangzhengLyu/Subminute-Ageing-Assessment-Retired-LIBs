...
%%% run repeated leave-one-out cross-validation using multi-output SVR
%%% (LIBSVM-based RBF-SVR with grid search for (C, gamma) per task), visualize
%%% train/test predictions, and save Y_Test for downstream evaluation.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for IndexData = 1:length(Feature_Nor)
            ...
            for i = 1:m
                for j = 1:n
                    ...
                    if cg(i,j) < error
                        error = cg(i,j);
                        bestc = 2^c(i,j);% select C
                        bestg = 2^g(i,j);% select gamma
                    end
                    if abs(cg(i,j) - error) <= eps && bestc > 2^c(i,j)% prioritize smaller C under tie
                        error = cg(i,j);
                        bestc = 2^c(i,j);
                        bestg = 2^g(i,j);
                    end
            ...
            % V. SVR prediction (train and test)
