...
%%% This script: Extract relaxation-voltage segments near a fixed setpoint,
%%% fit a double-exponential relaxation model, and assemble fitted RC-type
%%% relaxation parameters (Uoc, R0, R1, C1, R2, C2) for SDU-NCM (Part P2).
%%% The extracted features are saved to a MAT-file for downstream workflows.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for IndexData = 1:length(OneCycle)
   ...
   %% Relaxation-voltage extraction
    ...
    if ~ismember(IndexData,[])
        ...
        % Relaxation segment index (structure preserved for potential multi-segment use)
        CountRX = CountRX+1;

        % Drop initial points at the start of the relaxation segment (kept as in original)
        LengthDrop = 5; % LengthDrop = 0;
        LengthCops = 0;
        ...
        % Store relaxation voltage trace (note: indexing is kept exactly as original)
...
save Feature_ALL_SDU_NCM_P2.mat Uoc R0 R1 C1 R2 C2