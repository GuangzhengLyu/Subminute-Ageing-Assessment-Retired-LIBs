...
%%% evaluation using support vector regression (SVR, RBF kernel via libsvm).
%%% A grid search is performed for (C, gamma) for each task. Predictions are
%%% denormalized for visualization and saved to MAT-files for later analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:2
        ...
        for IndexData = 1:length(Feature_Nor)
            ...
            %% Model 1
            % 1. Search best C and gamma (grid search with cross-validation)
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1'];%2 Gaussian kernel; -v returns CV metric; -s 3 is SVR regression
