...
%%% evaluation using a Bayesian linear regression model. Predictions are
%%% denormalized for visualization and saved to MAT-files for downstream
%%% RMSE/statistics. Helper functions are included at the end of the script.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    % Get feature dimension and number of samples
    M = size(p_train, 1);
    N = size(p_train, 2);

    % Covariance assumptions (kept as in original function)
...
function t_sim = my_sim_bayes(model, p_train)
    % Compute model predictions (linear mapping)
    t_sim = p_train * model;
end