...
%%% This script: Assemble model inputs/outputs for SDU-NCM (Part P2),
%%% normalize relaxation-parameter features and multiple health indicators,
%%% and run repeated leave-one-out PLSR to predict six target indicators.
%%% Predicted outputs (Y_Test) are saved as MAT-files for later analysis.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Dimension meaning: Feature(sample, segmentIndex, featureIndex)
Feature(:,:,1) = Uoc;
Feature(:,:,2) = R0;
Feature(:,:,3) = R1;
Feature(:,:,4) = C1;
...
%% Normalize to health-indicator scale (relative to reference values/ranges)
% Note: Scaling factors and ranges are kept exactly as in the original script
Capa = Capa/2.4;
Life = Life;
ERate = ERate/0.95;
...
for CountSV = 14:-1:14 
    ...
    for CountRP = 1:100
        ...
        for IndexData = 1:length(Feature_Nor)
            ...
            ncomp = min(3, size(In_Train,1));  % Up to 5 components
    ...
    currentFile = sprintf('PLSR_Result_1_80_Y_Test_%d_SDU_NCM_P2.mat',CountSV);
