...
for IndexData = 1:length(OneCycle)
  ...
  %% Relaxation-voltage extraction
    ...
    if ~ismember(IndexData,[])
        ...
        % Drop the first points after current reaches zero to avoid transients
        LengthDrop = 20; % LengthDrop = 0;
        LengthCops = 0;
        ...
        % Store relaxation voltage trace (kept indexing as in original script)
...
for i = 1:-1:1
    ...
    figure(1),clf,hold on,box on,plot(Capa,Uoc(:,indexRX),'o')
    figure(2),clf,hold on,box on,plot(Capa,R0(:,indexRX),'d')
    figure(3),clf,hold on,box on,plot(Capa,R1(:,indexRX),'<')
    figure(4),clf,hold on,box on,plot(Capa,C1(:,indexRX),'>')
    figure(5),clf,hold on,box on,plot(Capa,R2(:,indexRX),'*')
    figure(6),clf,hold on,box on,plot(Capa,C2(:,indexRX),'+')
end

% Save assembled relaxation-feature matrices for downstream modelling
save Feature_ALL_RWTH_NCA.mat Uoc R0 R1 C1 R2 C2