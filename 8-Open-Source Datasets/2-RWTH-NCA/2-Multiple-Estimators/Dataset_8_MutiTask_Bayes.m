...
%%% multi-task Bayesian linear regression (single Bayesian model for all
%%% outputs). Predictions are clipped, de-normalized, visualized, and saved
%%% to MAT-files for downstream benchmarking and comparison.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %% Input feature normalization
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
          ...
          %% Bayesian linear regression (multi-output)
            % Hyperparameters
            sigma_squared = 0.01; % output noise covariance (variance term)
            eta_sqaured   = 0.02; % prior covariance (variance term) for coefficients

            % Train Bayesian regression model (shared by all outputs)
...
function mu = my_bayesian_regression(p_train, t_train, small_sigma_squared, eta_sqaured)
    ...
    % Covariance matrices (kept as in original implementation)
...
function t_sim = my_sim_bayes(model, p_train)
    % Predict outputs using the fitted coefficient matrix
    t_sim = p_train * model;
end