...
%%% multi-task GPR (one GPR model per task). Predictions are clipped,
%%% de-normalized, visualized, and saved to MAT-files for downstream
  ...
  %% Input feature normalization
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Train one GPR model per task (kept as original)
            % Note: The original comment said KNN; here it is corrected to GPR.
