...
%%% multi-task extreme learning machine (ELM) regression. An ELM is trained
%%% per repetition on the leave-one-out training set, predictions are clipped
%%% and de-normalized, results are visualized, and saved to MAT-files for
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
          ...
          %% Extreme Learning Machine (ELM)
            % ELM hyperparameters
            num_hiddens = 10;        % number of hidden neurons
            activate_model = 'sig';  % activation function

            % Train ELM (single hidden-layer random feature model)
