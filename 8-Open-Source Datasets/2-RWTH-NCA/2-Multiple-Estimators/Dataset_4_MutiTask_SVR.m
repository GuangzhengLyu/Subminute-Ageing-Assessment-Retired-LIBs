...
%%% multi-task SVR (one SVR model per task, RBF kernel). Hyperparameters
%%% (C, gamma) are selected via grid search with cross-validation. Predictions
%%% are clipped, de-normalized, visualized, and saved to MAT-files for
%%% downstream benchmarking and comparison.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% SVR library functions (LIBSVM-based)
addpath('./4-SVR');
  ...
  %% Input feature normalization
    ...
    for CountRP = 1:2
        for IndexData = 1:length(Feature_Nor)
          ...
          %% Model 1
            % 1) Search best C and gamma by grid search with cross-validation
            ...
            for i = 1:m
                for j = 1:n
                    cmd = ['-v ',num2str(v),' -t 2',' -c ',num2str(2^c(i,j)),' -g ',num2str(2^g(i,j) ),' -s 3 -p 0.1']; % 2: RBF; -s 3: SVR
                    ...
                    if abs(cg(i,j) - error) <= eps && bestc > 2^c(i,j) % prioritize smaller C
                        error = cg(i,j);
                        bestc = 2^c(i,j);
                        bestg = 2^g(i,j);
                    end
            ...
            % 2) Train the SVR model using best C and gamma
