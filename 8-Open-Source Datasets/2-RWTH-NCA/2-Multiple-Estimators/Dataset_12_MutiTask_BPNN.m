...
%%% multi-task backpropagation neural network (BPNN) regression. A feedforward
%%% network is trained per repetition on the leave-one-out training set,
%%% predictions are clipped and de-normalized, results are visualized, and
%%% saved to MAT-files for downstream benchmarking and comparison.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
          ...
          %% Backpropagation Neural Network (BPNN)
            ...
            % Training hyperparameters (kept as in the original script)
            net.trainParam.epochs = 100000000;
            net.trainParam.lr = 0.001;
            net.trainParam.goal = 0.0001;

            % Train the network on the leave-one-out training set
            net = train(net,In_Train,Out_Train);

            % Predict (training set and held-out sample)
