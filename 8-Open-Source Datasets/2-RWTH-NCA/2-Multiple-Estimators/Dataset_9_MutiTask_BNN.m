...
%%% multi-task Bayesian-regularized neural network regression (trainbr).
%%% Predictions are clipped, de-normalized, visualized, and saved to MAT-files
%%% for downstream benchmarking and comparison.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
          ...
          %% Bayesian-regularized neural network (trainbr)
            % Create Bayesian regularization network (two hidden layers)
            net = fitnet([10, 5], 'trainbr');  % two hidden layers: 10 and 5 neurons
            net.trainParam.showWindow = false; % disable training GUI for batch runs
            net.trainParam.epochs = 20;        % keep original epoch setting
