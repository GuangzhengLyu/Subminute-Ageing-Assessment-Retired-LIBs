...
%%% Dataset: 2-RWTH-NCA
%%% This script: Perform grouped K-means clustering using multi-group ageing
%%% information (estimation outputs and relaxation features), visualize the
%%% clustering in PCA space, and quantify within-cluster trajectory
%%% dispersion using cycle-wise standard deviations of key signals. Summary
%%% ratios and bar plots are generated for comparing different input-group
%%% combinations (Kind = 1..4).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
load('./PLSR_Result_1_80_Y_Test_14_RWTH_NCA.mat')

% Use the mean prediction over repeated runs as the final estimation
...
for Kind = 1:4
    ...
    % Group 1: capacity-related estimation (N×1)
    Xg{1} = Estimation(:,1);

    % Group 2: life/RUL-related estimation (N×1; optionally enabled by Kind)
    Xg{2} = [Estimation(:,2)];
    ...
    % Group 3: relaxation feature vector (N×6; optionally enabled by Kind)
    ...
    % Group 4: expanded indicators (N×4; optionally enabled by Kind)
    ...
    %% ========== 2. Pre-processing: group-wise z-score normalization ==========
    Xg_norm = cell(G,1);
    for g = 1:G
        [Xg_norm{g}, ~] = zscore_norm(Xg{g});
    end

    % Reassemble the full feature matrix for K-means (manual concatenation)
    % This avoids cell2mat issues when dimensions are inconsistent.
    Xn = [];
    for g = 1:numel(Xg_norm)
        ...
        % If it accidentally becomes d×N, transpose to N×d
        ...
        if size(A,1) ~= N
            error('Group %d has incorrect dimensions: expected %d×%d, but got %d×%d', ...
    ...
    % Group weights: set per-group importance (kept as in original script)
    w = [1, 1, 1, 1];
    ...
    title('K-means clustering (performed in full space; PCA for visualization only)');

    %% ========== 5. Visualization 2: cluster-center profiles by group ==========
    % Split C back into groups and plot the center profiles (normalized space)
    ...
    for g = 1:G
        ...
        grid on;
        xlabel(sprintf('Group %d feature-dimension index', g));
    ...
    %% ========== 7. Trajectory-dispersion metrics within clusters ==========
    % Compute cycle-wise standard deviations for several trajectories/signals
    for i = 1:K
    ...
    % Clear per-index intermediate variables
    clear DiscCapa DiscEngy CoChCapa CharTime PlfDCapa Length % variables associated with index j
    clear STD_DiscCapa STD_DiscEngy STD_CoChCapa STD_CharTime STD_PlfDCapa % variables associated with index m
end
...
% Summarize relative dispersion (normalized by Kind = 1 baseline)
...
% Bar plots for each dispersion metric
