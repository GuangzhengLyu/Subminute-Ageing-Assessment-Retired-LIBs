...
%%% -> PLSR multi-task ageing assessment on SCU3 Dataset #3 using repeated
%%% leave-one-out per voltage setpoint (3.0 V to 4.2 V).
%%% Note: For Dataset #3, training samples for each test cell exclude cells
%%% from the same tester channel to avoid information leakage in RUL-related
%%% prediction.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for CountSV = 13:-1:1
    ...
    Max_In = [4,    0.16, 0.09,  2500,0.05,  2500,  0.03,  1200, 0.025, 1200;
              3.91, 0.17, 0.1,   2200,0.06,  2000,  0.028, 900,  0.022, 1800;
              3.83, 0.17, 0.11,  2200,0.06,  1700,  0.026, 1000, 0.025, 1500;
              3.73, 0.18, 0.1,   2000,0.055, 2000,  0.035, 1000, 0.03,  2200;
              3.6,  0.17, 0.1,   2000,0.05,  1800,  0.035, 1000, 0.03,  1800;
              3.5,  0.17, 0.085, 2000,0.045, 1600,  0.035, 1000, 0.025, 2000;
              3.35, 0.17, 0.075, 2000,0.07,  1500,  0.04,  700,  0.035, 900;
              3.26, 0.17, 0.055, 2200,0.05,  1600,  0.04,  500,  0.03,  550;
              3.17, 0.17, 0.041, 2000,0.04,  2000,  0.04,  1600, 0.03,  550;
              3.08, 0.155,0.03,  2500,0.035, 1500,  0.03,  3500, 0.025, 3000;
              2.99, 0.14, 0.035, 1500,0.025, 1000,  0.022, 1300, 0.016, 1500;
              2.94, 0.13, 0.035, 550, 0.018, 1000,  0.012, 1000, 0.012, 1000;
              2.9,  0.11, 0.011, 560, 0.011, 550,   0.011, 550,  0.011, 550];

    Min_In = [3.65, 0.05, 0.025, 500, 0.01,  600,   0.005, 0,    0,     100;
              3.55, 0.05, 0.02,  500, 0.01,  400,   0.004, 0,    0,     0;
              3.45, 0.05, 0.02,  450, 0.01,  400,   0.005, 0,    0,     0;
              3.35, 0.06, 0.02,  450, 0.01,  300,   0.005, 0,    0,     0;
              3.23, 0.05, 0.02,  400, 0.015, 0,     0.005, 0,    0,     0;
              3.17, 0.06, 0.03,  400, 0.018, 0,     0.005, 0,    0,     0;
              3.1,  0.07, 0.028, 400, 0.015, 0,     0.005, 80,   0,     50;
              3.03, 0.07, 0.02,  400, 0.01,  100,   0.005, 50,   0.002, 50;
              2.98, 0.07, 0.018, 0,   0.014, 0,     0.008, 0,    0.002, 50;
              2.93, 0.08, 0.015, 0,   0.01,  100,   0.006, 0,    0,     0;
              2.89, 0.08, 0.007, 0,   0.007, 0,     0,     100,  0,     0;
              2.84, 0.04, 0.004, 100, 0.003, 100,   0.002, 200,  0.002, 100;
              2.78, 0.02, 0.002, 200, 0.002, 200,   0.002, 200,  0.002, 200];
    ...
    %% Repeated leave-one-out PLSR (channel-exclusion for Dataset #3)
    for CountRP = 1:100
        for IndexData = 1:length(Feature_Nor)
            ...
            % Channel exclusion: remove all samples from the same channel as the test cell
            CountSamChannel = 0;
...
%         end
    ...
    currentFile = sprintf('PLSR_Result_3_50_Y_Test_%d_4RC.mat',CountSV);
