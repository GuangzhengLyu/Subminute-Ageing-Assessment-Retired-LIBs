...
%%% This script: Evaluate 5-RC PLSR prediction results for SCU3 Dataset #1.
%%% It rebuilds the normalized targets (capacity SOH, life index, and four
%%% expanded health indicators) from OneCycle_1_260102.mat, loads the saved
%%% per-setpoint prediction tensors (13 setpoints, 100 repeats each), then
%%% computes RMSE distributions for each target. A normalized multi-task
%%% score is formed by dividing per-task RMSE by fixed scale factors and
%%% averaging across tasks to compare setpoint sensitivity.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% SCU3 Dataset #1
% Load structured single-cycle dataset (updated snapshot)
...
for IndexData = 1:length(OneCycle)
    if OneCycle(IndexData).Steps(end) == 30
        ...
        % Life definition (threshold-based)
        ...
        % Normalized capacity SOH proxy
        Capa(CountData,1) = OneCycle(IndexData).OrigCapaAh./3.5;

        % Expanded health indicators (normalized to fixed engineering ranges)
...
%% Result aggregation (RMSE per setpoint, per repeat)
% Load saved prediction tensors:
%   Y_Test(repeat_index, output_index, sample_index)
for i = 1:13
    currentFile = sprintf('./PLSR_Result_1_70_Y_Test_%d_5RC.mat',i);
    load(currentFile)
...
%% Example parity/error plots (uses the last loaded setpoint and last repeat index)
% Note: "j" here remains 100 after the above loops, matching the original logic.
...
%% Setpoint axis (3.0–4.2 V, 0.1 V step)
Uter = 3.0:0.1:4.2;

%% Normalized multi-task RMSE score (dataset-specific scaling factors)
...
% Average across tasks to obtain a single setpoint sensitivity curve
MyTemp = mean(Temp,1);
